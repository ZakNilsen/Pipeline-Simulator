...
int exec(statetype* state, statetype* newstate){
	...
	newstate->EXMEM.readreg      = regAHold;
		...
		newstate->EXMEM.aluresult = ~(regAHold & regBHold);
                ...
                //aluresult = rB+offset
		newstate->EXMEM.aluresult = regBHold + state->IDEX.offset;
	}else if(op==SW){
		//aluresult = rB+offset
		newstate->EXMEM.aluresult = regBHold + state->IDEX.offset;
	}else if(op==BEQ){
...
void writeBack(statetype* state, statetype* newstate){
	...
	if(op==ADD || op==NAND){
		...
		//sets destReg
		int destReg = field0(state->MEMWB.instr);
		...
		//datamem[rB+offset] = writedata = contents of rA
		newstate->datamem[state->reg[field1(state->MEMWB.instr)]+
					signExtend(field2(state->MEMWB.instr))]=
